...
class PW_API BlockMoreDimension : public BlockBase
{
    Q_OBJECT
    ...
    Q_PROPERTY(QStringList UnitList READ GetUnitList WRITE SetUnitList)
    Q_PROPERTY(bool IsDisplayDouble READ GetIsDisplayDouble WRITE SetIsDisplayDouble)

public:
    /**
     * @brief 构造函数，用于创建一个 BlockMoreDimension 对象。
     *
     * @param parent 指向父窗口部件的指针，默认为 nullptr。
     */
    explicit BlockMoreDimension(QWidget *parent = nullptr);

    /**
     * @brief 析构函数，用于销毁一个 BlockMoreDimension 对象。
     */
    ~BlockMoreDimension();
    ...
    QStringList GetUnitList() const;

    void SetUnitList(const QStringList &unit_list);

    void SetIsDisplayDouble(bool is_display);

    bool GetIsDisplayDouble() const;
...
private slots:
    void OnEnumIndexChanged();
private:
    PowerDoubleLineEdit *m_double;
    PowerEnumeration *m_unit;
    QStringList m_unit_list{};
    bool m_is_display_double = true;
};
