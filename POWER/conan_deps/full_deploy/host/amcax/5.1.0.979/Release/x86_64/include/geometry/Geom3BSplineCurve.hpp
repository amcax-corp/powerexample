...
/// @brief     Class of 3D B spline curve
...
{
/// @brief Class of 3D B spline curve
class AMCAX_CLASS_API Geom3BSplineCurve : public Geom3BoundedCurve
{
public:
	/// @brief Default constructor
	/// @details A default curve is a line from the origin towards x-axis with length 1.
	AMCAX_API Geom3BSplineCurve();
	...
	AMCAX_API Geom3BSplineCurve(const std::vector<Point3>& pts, const std::vector<double>& knots, const std::vector<int>& multiplicities, int degree, bool isPeriodic = false);
	...
	/// @param degree The degree of curve
	/// @param isPeriodic Set the curve is periodic or not
	/// @param checkRational To check whether the weights are rational
	AMCAX_API Geom3BSplineCurve(const std::vector<Point3>& pts, const std::vector<double>& weights, const std::vector<double>& knots, const std::vector<int>& multiplicities, int degree, bool isPeriodic = false, bool checkRational = true);
	...
	AMCAX_API void SetPole(int i, const Point3& p);

	/// @brief Set the pole and weight at a given index
	/// @param i The given index
	/// @param p The new pole
	/// @param weight The new weight
	AMCAX_API void SetPole(int i, const Point3& p, double weight);
	...
	AMCAX_API void MovePoint(double u, const Point3& p, int i1, int i2, int& firstModifiedPole, int& lastModifiedPole);

	AMCAX_API Point3 StartPoint() const override;
	AMCAX_API Point3 EndPoint() const override;
	...
	/// @return The point
	AMCAX_API Point3 LocalValue(double u, int fromK1, int toK2) const;
	...
	AMCAX_API void LocalD0(double u, int fromK1, int toK2, Point3& p) const;
	...
	AMCAX_API void LocalD1(double u, int fromK1, int toK2, Point3& p, Vector3& v1) const;
	...
	AMCAX_API void LocalD2(double u, int fromK1, int toK2, Point3& p, Vector3& v1, Vector3& v2) const;
	...
	AMCAX_API void LocalD3(double u, int fromK1, int toK2, Point3& p, Vector3& v1, Vector3& v2, Vector3& v3) const;
	...
	AMCAX_API Vector3 LocalDN(double u, int fromK1, int toK2, int n) const;
	...
	AMCAX_API const Point3& Pole(int i) const;
	...
	/// @brief Does the B spline curve is equal to the other B spline curve
	/// @param other The other B spline curve
	/// @param tol The tolerance
	/// @return True if two B spline curves are equal under a tolerance
	AMCAX_API bool IsEqual(const std::shared_ptr<Geom3BSplineCurve>& other, double tol) const;

private:
	void UpdateKnots();
	static void CheckCurveData(const std::vector<Point3>& poles, const std::vector<double>& knots, const std::vector<int>& mults, int degree, bool periodic);
