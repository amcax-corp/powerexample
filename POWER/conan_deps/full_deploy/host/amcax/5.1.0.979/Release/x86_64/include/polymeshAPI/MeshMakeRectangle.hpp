...
/// @brief     Class of PolyMesh API for make a plane rectangle
...
{
/// @brief Class of PolyMesh API for make a plane rectangle
class MeshMakeRectangle
{
public:
	/// @brief Construct a rectangle from [0, 1] x [0, 1];
	AMCAX_API MeshMakeRectangle();
	...
	AMCAX_API MeshMakeRectangle(const Point2& p0, const Point2& p1, size_t useg = 2, size_t vseg = 2);
	...
	/// @param useg The number of segments in x direction
	/// @param vseg The number of segments in y direction
	AMCAX_API MeshMakeRectangle(const Frame3& frame, double dx, double dy, size_t useg = 2, size_t vseg = 2);

	/// @brief Build and get the result PolyMesh
	AMCAX_API PolyMesh* BuildMesh();

private:
	void MakeRectangleImp(PolyMesh*& mesh);
