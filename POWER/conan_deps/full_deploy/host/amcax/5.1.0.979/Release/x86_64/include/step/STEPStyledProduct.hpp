/// @file       STEPStyledProduct.hpp
/// @brief		Represents a product in the STEP standard with geometry and style information.
...
{
/// @brief Represents a product in the STEP standard with geometry and style information.
class AMCAX_CLASS_API STEPStyledProduct : public PNode<STEPStyledProduct>
{
public:
	/// @brief Default constructor. All members are initialized as empty.
	AMCAX_API STEPStyledProduct();
	...
	AMCAX_API explicit STEPStyledProduct(const TopoShape& s,
		const std::string& n = std::string());

	/// @brief Copy constructor.
	/// @param other The STEPStyledProduct object to copy.
	AMCAX_API explicit STEPStyledProduct(const STEPStyledProduct& other);

	/// @brief Copy constructor from shared pointer.
	/// @param other Shared pointer to the STEPStyledProduct object to copy.
	AMCAX_API explicit STEPStyledProduct(const std::shared_ptr<STEPStyledProduct>& other);

	/// @brief Move constructor.
	/// @param other The STEPStyledProduct object to move from.
	AMCAX_API explicit STEPStyledProduct(STEPStyledProduct&& other) noexcept;

	/// @brief Virtual destructor.
	AMCAX_API virtual ~STEPStyledProduct() noexcept;
	...
	/// @brief Get style properties for all shapes.
	/// @details Delegates to the target node if this is a shadow.
	/// @return Const reference to the list of style maps.
	AMCAX_API const std::vector<std::unordered_map<TopoShape, ShapeProperty>>& Properties() const;

	/// @brief Get the number of style property maps.
	/// @details Delegates to the target node if this is a shadow.
	/// @return Number of property maps.
	AMCAX_API size_t PropertiesSize() const;

	/// @brief Get style properties for a specific shape.
	/// @details Delegates to the target node if this is a shadow.
	/// @param index Index corresponding to the shape.
	/// @return Const reference to the style map.
	...
	/// @brief Get style properties owned by this node.
	/// @return Const reference to the owned style maps.
	AMCAX_API const std::vector<std::unordered_map<TopoShape, ShapeProperty>>& OwnedProperties() const;

	/// @brief Get the number of owned style property maps.
	/// @return Number of owned maps.
	AMCAX_API size_t OwnedPropertiesSize() const;

	/// @brief Get specific owned style properties by index.
	/// @param index Index of the style map.
	/// @return Const reference to the style map at the given index.
	...
	AMCAX_API const std::vector<std::shared_ptr<STEPStyledProduct>>& Children() const;
	...
	AMCAX_API const std::shared_ptr<STEPStyledProduct>& ChildAt(size_t index) const;

	/// @overload
	AMCAX_API std::shared_ptr<STEPStyledProduct>& ChildAt(size_t index);

	/// @brief Add a child product node.
	/// @details Delegates to the target node if this is a shadow.
	/// @param child The child node to add.
	AMCAX_API void AddChild(const std::shared_ptr<STEPStyledProduct>& child);
	...
	AMCAX_API const std::vector<std::shared_ptr<STEPStyledProduct>>& OwnedChildren() const;
	...
	AMCAX_API const std::shared_ptr<STEPStyledProduct>& OwnedChildAt(size_t index) const;

	/// @overload
	AMCAX_API std::shared_ptr<STEPStyledProduct>& OwnedChildAt(size_t index);

	/// @brief Add a child node directly to this product node.
	/// @param child The child node to add.
	AMCAX_API void AddOwnedChild(const std::shared_ptr<STEPStyledProduct>& child);
	...
	/// @param properties Shape properties (style data).
	...
	AMCAX_API void AddShape(const TopoShape& shape,
		const std::unordered_map<TopoShape, ShapeProperty>& properties = std::unordered_map<TopoShape, ShapeProperty>{},
	...
	/// @details Delegates to the target node if this is a shadow.
	/// @note Also removes all properties, labels, factors, and representation types.
	...
	AMCAX_API const std::shared_ptr<STEPStyledProduct>& Target() const;
	...
	/// @param clear If true, clears local shapes, properties, etc.
	AMCAX_API void MarkShadow(const std::shared_ptr<STEPStyledProduct>& target, bool clear = false);
	...
	AMCAX_API std::shared_ptr<STEPStyledProduct> EmptyCopy() const;
	...
	/// @return Number of representation item entries.
	...
	/// @brief Set (replace) the entire representation items map.
	...
	/// @return Number of owned entries.
	...
	/// @brief Set (replace) the entire owned representation items map.
	/// @param items The new items map.
	...
	AMCAX_API static std::vector<std::shared_ptr<STEPStyledProduct>> DeepCopy(const std::vector<std::shared_ptr<STEPStyledProduct>>& orig);

	/// @brief Create a composite TopoShape from a list of STEPStyledProduct nodes.
	/// @param nodes List of STEPStyledProduct nodes.
	/// @return The composite TopoShape.
	AMCAX_API static TopoShape OneShape(const std::vector<std::shared_ptr<STEPStyledProduct>>& nodes);

private:
	std::vector<std::unordered_map<TopoShape, ShapeProperty>> m_properties;
};
} // namespace AMCAX::STEP
