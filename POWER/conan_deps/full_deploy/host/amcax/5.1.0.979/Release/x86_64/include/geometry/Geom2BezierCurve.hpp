...
/// @brief     Class of 2D Bezier curve
...
{
/// @brief Class of 2D Bezier curve
class AMCAX_CLASS_API Geom2BezierCurve : public Geom2BoundedCurve
{
public:
	/// @brief Default constructor
	/// @details A default curve is a line from the origin towards x-axis with length 1.
	AMCAX_API Geom2BezierCurve();

	/// @brief Construct a bezier curve from poles
	/// @param pts The poles
	AMCAX_API explicit Geom2BezierCurve(const std::vector<Point2>& pts);

	/// @brief Construct a rational bezier curve from poles and weights
	/// @param pts The poles
	/// @param w The weights
	AMCAX_API Geom2BezierCurve(const std::vector<Point2>& pts, const std::vector<double>& w);
	...
	AMCAX_API void InsertPole(int index, const Point2& p, double weight = 1.0);
	...
	/// @param p The point
	AMCAX_API void SetPole(int index, const Point2& p);

	/// @brief Set the pole and weight at a given index
	/// @param index The given index
	/// @param p The pole
	/// @param weight The weight
	AMCAX_API void SetPole(int index, const Point2& p, double weight);
	...
	AMCAX_API Point2 StartPoint() const noexcept override;
	AMCAX_API Point2 EndPoint() const noexcept override;
	...
	AMCAX_API const Point2& Pole(int index) const;
	...
	/// @param[in] toluv The 2D tolerance
	/// @param[out] utol The parametric tolerance
	AMCAX_API void Resolution(double toluv, double& utol);
...
private:
	void Init(const std::vector<Point2>& pts, const std::vector<double>& ws);
	static bool IsRational(const std::vector<double>& w) noexcept;
	static bool IsClosed(const std::vector<Point2>& pts) noexcept;
