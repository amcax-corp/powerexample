 ...
 */
template <typename value_type, typename key_type = index_t,
          typename comparator = std::less<value_type>>
...
public:
	...
	void push(key_type key, const value_type &value)
	{
		...
		{
			AMCAXMeshing_ExpensiveAssert(!exist(key), "Add duplicate key into heap.");
		}

		m_heap.emplace_back(key, value);
		if (static_cast<size_t>(key) < m_pos_in_heap.size())
		{
			m_pos_in_heap[key] =
			  static_cast<key_type>(heap_size()); // start from 1, not 0.
		}
		else
		{
			m_pos_in_heap.resize(key + 1, InvalidIndex);
			m_pos_in_heap[key] = static_cast<key_type>(heap_size());
		}
		up_move(heap_size());
	}
	...
	{
		AMCAXMeshing_ExpensiveAssert(!empty(), "Heap is empty.");
	...
	{
		return is_valid_idx(key) &&
		       static_cast<size_t>(key) < m_pos_in_heap.size() &&
	...
	{
		AMCAXMeshing_ExpensiveAssert(exist(key),
	...
	void remove(size_t key)
	{
		...
		m_pos_in_heap[m_heap[pos].first] = static_cast<key_type>(pos);
		m_heap.pop_back();
...
protected:
	...
	void up_move(index_t idx)
	{
		...
		while (p >= first_pos() && compare(empty_pos(), p))
		{
			...
			m_pos_in_heap[m_heap[idx].first] = static_cast<key_type>(idx);
