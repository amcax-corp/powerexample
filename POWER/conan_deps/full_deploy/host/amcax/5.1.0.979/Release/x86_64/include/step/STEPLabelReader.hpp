/// @file       STEPLabelReader.hpp
/// @brief		Reader class for translating STEP files into STEPLabelProduct trees.
...
/// @brief Reader class for translating STEP files into STEPLabelProduct trees.
class AMCAX_CLASS_API STEPLabelReader : public BReader<STEPLabelProduct>
{
public:
	/// @brief Construct a reader from an input stream.
	/// @param is Input stream containing STEP data.
	AMCAX_API STEPLabelReader(std::istream& is);

	/// @brief Construct a reader from a file path.
	/// @param filename Path to the STEP file.
	AMCAX_API STEPLabelReader(std::string filename);

	/// @brief Destructor.
	AMCAX_API virtual ~STEPLabelReader();

	/// @brief Specify the style level used by the reader.
	/// @param level The style level.
	/// @note If NONE, the reader behaves similarly to STEPReader.
	///       This setting also affects AMCAX::AF attributes.
	...
	/// @brief Set the base length unit for translation (default is 1.0, millimetres).
	...
	AMCAX_API bool Read(STEPDataCallback<STEPLabelProduct> callback = nullptr);

	/// @brief Get the list of STEPLabelProduct trees parsed from the STEP file.
	/// @note The value is only valid after Read() is done.
	/// @return Constant reference to the list of STEPLabelProduct nodes.
	AMCAX_API const std::vector<std::shared_ptr<STEPLabelProduct>>& GetProducts() const;

	/// @overload
	AMCAX_API std::vector<std::shared_ptr<STEPLabelProduct>>& GetProducts();
...
private:
	ShapeStyleLevel m_slevel{ShapeStyleLevel::FACE};
};
} // namespace STEP
} // namespace AMCAX
...
namespace STEP
{
extern template class BReader<STEPLabelProduct>;
} // namespace STEP
} // namespace AMCAX
